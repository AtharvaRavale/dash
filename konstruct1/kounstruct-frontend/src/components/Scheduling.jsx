...
// } from "../api";
// import { NEWchecklistInstance } from "../api/axiosInstance"; // ✅ for list endpoints (fallbacks)
...
//   // ---- Load setup (structure/users/purposes) ----
//   useEffect(() => {
//     if (!projectId) return;
//     const load = async () => {
//       try {
...
//         const norm = arr
//           .map((c) => ({
//             id: Number(c?.id ?? c?.checklist_id ?? c?.pk),
//             name: c?.name ?? c?.title ?? `Checklist #${c?.id}`,
//           }))
...
//     // ✅ If strict (some flat selected) => must match selected flats only
//     const flatOk = anyFlatSelected ? flatIdSet.has(Number(c.flat_id)) : true;

//     // ✅ if not strict (no flat selected) => show all of selected floors (targets based)
//     const floorOk = !anyFlatSelected
//       ? flatIdSet.has(Number(c.flat_id)) // because targets contains ALL flats of selected floors
//       : true;

//     return scopeOk && flatOk && floorOk;
//   });

//   // ✅ decorate to show Flat + Room in dropdown (optional but helps)
//   const decorated = scoped.map((c) => ({
//     ...c,
//     name: `Flat:${c.flat_id} • Room:${c.room_id ?? "-"} • ${c.name}`,
//   }));
...
//   if (!decorated.length) toast("No checklists found for selected flat(s)", { icon: "ℹ️" });
// };
const applyScopeFilters = () => {
  if (!selectedPurposeId || !selectedPhaseId || !selectedStageId) {
    toast.error("Please select Purpose + Phase + Stage first");
...
//   // ✅ Must pick location before Apply
//   if (mappingsPreview.length === 0) {
//     toast.error("Please select Tower + Floor (and Flat if needed) first");
//     return;
//   }
...
//   // ✅ create a fast lookup set of target keys
//   const targetKeySet = new Set(
//     targets.map((t) => `${t.building_id}-${t.level_id}-${t.flat_id}`)
//   );

//   // ✅ Filter checklists by scope + selected flats (room-wise list should come from backend)
//   const scoped = allProjectChecklists.filter((c) => {
//     const scopeOk =
//       Number(c.purpose_id) === pId &&
//       Number(c.phase_id) === phId &&
...
//     // if backend doesn't send target ids, we cannot filter by flat
//     const hasTargetIds =
//       Number.isFinite(c.building_id) && Number.isFinite(c.level_id) && Number.isFinite(c.flat_id);

//     const targetOk = hasTargetIds
//       ? targetKeySet.has(`${c.building_id}-${c.level_id}-${c.flat_id}`)
//       : true; // fallback (will show many if backend missing ids)

//     return scopeOk && targetOk;
//   });

//   // ✅ Make checklist name more informative (flat/room)
//   const decorated = scoped.map((c) => ({
//     ...c,
//     name:
//       (c.flat_number || c.flat_id ? `Flat ${c.flat_number ?? c.flat_id}` : "") +
//       (c.room_name ? ` • ${c.room_name}` : "") +
//       ` • ${c.name}`,
//   }));

//   setChecklists(decorated);
//   setSelectedChecklistIds([]); // keep empty until user selects or Select All
...
//   if (!decorated.length) toast("No checklists found for this flats/rooms", { icon: "ℹ️" });
//   if (!onlyCheckers.length) toast("No CHECKER users found", { icon: "ℹ️" });
// };
...
//   // ✅ Filter checklists by scope
//   const scoped = allProjectChecklists.filter(
//     (c) =>
//       Number(c.purpose_id) === pId &&
//       Number(c.phase_id) === phId &&
//       Number(c.stage_id) === stId
...
//   if (!scoped.length) toast("No checklists found for this scope", { icon: "ℹ️" });
//   if (!onlyCheckers.length) toast("No CHECKER users found", { icon: "ℹ️" });
// };
...
<div className="flex items-end">
  {/* <button
    ...
    disabled={!selectedPurposeId || !selectedPhaseId || !selectedStageId}
    style={{
      opacity:
        !selectedPurposeId || !selectedPhaseId || !selectedStageId ? 0.5 : 1,
    }}
  >
    Apply
  </button> */}
